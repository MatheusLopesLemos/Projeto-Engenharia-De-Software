...
public class Professor {
	...
	final private static int TAM_ESP = 20;
	...
	@Column(length = TAM_ESP)
	private String especialidade;
	@OneToMany(mappedBy="professor") //--> mappedBy indica qual é o nome do atributo unário presente na outra classe
	...
	public Professor(String c, String n, String d, String e, String t, String esp) throws ModelException {
		super();
		this.setCpf(c);
		this.setNome(n);
		this.setDataNasc(d);
	...
	public static void validarEspecialidade(String esp) throws ModelException {
		...
		if (esp == null || esp.length() == 0)
			throw new ModelException("especialidade não pode ser nulo");

		int tamanho = esp.length();

		if (tamanho > TAM_ESP)
			throw new ModelException("especialidade deve ter até 20 caracteres");

		for (int i = 0; i < tamanho; i++) {
			char cp = esp.charAt(i);
			if (!Character.isAlphabetic(cp))
				throw new ModelException("especialidade deve possuir apenas letras");
	...
	public String toString() {
		return "CPF_Professor : " + this.cpf + " " + "Nome_Professor: " + this.nome + " " + "Especialidade: "
				+ this.especialidade;
	}
